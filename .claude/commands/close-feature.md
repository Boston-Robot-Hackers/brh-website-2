...
1. Find `process/03-features/notdone/FNN-*.md` and
   `process/04-tasks/notdone/TFNN-*.md` for that number. If either is missing,
   stop and report it.
...
4. Move the task file from `process/04-tasks/notdone/` to `process/04-tasks/done/`.
5. On the feature file, set `Done: yes`, `Tests Written: yes`, `Test Passing: yes`.
6. Move the feature file from `process/03-features/notdone/` to
   `process/03-features/done/`.
